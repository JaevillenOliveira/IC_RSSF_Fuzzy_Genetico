...
void JitterFilter::receiveSignal(cResultFilter *prev, simtime_t_cref t, cObject *object, cObject *details)
{
    if (auto packet = dynamic_cast<Packet *>(object)) {
        for (auto& region : packet->peekData()->getAllTags<CreationTimeTag>()) {
            ...
            if(prevDelay == -1)
                prevDelay = t - region.getTag()->getCreationTime();
            else{
                fire(this, t, fabs(prevDelay - (t - region.getTag()->getCreationTime())), &weight);
                prevDelay = t - region.getTag()->getCreationTime();
            }
        }
    }
